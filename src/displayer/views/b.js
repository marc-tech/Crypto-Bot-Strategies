...
const socketHeartbeat = () => {
  ...
  for (let endpointId in subscriptions) {
    ...
    if (ws.isAlive) {
      ...
      if (options.verbose)
        options.log('Terminating inactive/broken WebSocket: ' + ws.endpoint);
...
const handleSocketClose = function(reconnect, code, reason) {
  ...
  if (options.reconnect && this.reconnect && reconnect) {
    if (this.endpoint && parseInt(this.endpoint.length, 10) === 60)
      options.log('Account data WebSocket reconnecting...');
...
const futuresSocketHeartbeat = () => {
  ...
  for (let endpointId in futuresSubscriptions) {
    ...
    if (ws.isAlive) {
      ...
      if (options.verbose)
        options.log(`Terminating zombie futures WebSocket: ${ws.endpoint}`);
      if (ws.readyState === openState) ws.close();
    }
  }
};
...
const futuresKlineConcat = (symbol, interval) => {
  ...
  if (typeof futuresRealtime[symbol][interval].time === 'undefined')
    return output;
...
const futuresKlineHandler = (symbol, kline, firstTime = 0) => {
  ...
  if (!isFinal)
    return (futuresRealtime[symbol][interval] = {
      time,
      closeTime,
      open,
      high,
  ...
  futuresTicks[symbol][interval][time] = {
    time,
    closeTime,
    open,
    high,
...
const futuresKlineData = (symbol, interval, ticks) => {
  let last_time = 0;
  if (options.verbose)
    console.info('futuresKlineData', symbol, interval, ticks);
  if (isIterable(ticks)) {
    for (let tick of ticks) {
      // eslint-disable-next-line no-unused-vars
      let [
...
window.binance = {
  ...
  futuresAggTradeStream: function futuresAggTradeStream(symbols, callback) {
    let reconnect = () => {
      if (options.reconnect) futuresAggTradeStream(symbols, callback);
    };
    let subscription,
  ...
  futuresChart: async function futuresChart(
    ...
    let reconnect = () => {
      if (options.reconnect) futuresChart(symbols, interval, callback, limit);
    };

    let futuresChartInit = symbol => {
      if (typeof futuresMeta[symbol] === 'undefined') futuresMeta[symbol] = {};
      if (typeof futuresMeta[symbol][interval] === 'undefined')
        futuresMeta[symbol][interval] = {};
      if (typeof futuresTicks[symbol] === 'undefined')
        futuresTicks[symbol] = {};
      if (typeof futuresTicks[symbol][interval] === 'undefined')
        futuresTicks[symbol][interval] = {};
      if (typeof futuresRealtime[symbol] === 'undefined')
        futuresRealtime[symbol] = {};
      if (typeof futuresRealtime[symbol][interval] === 'undefined')
        futuresRealtime[symbol][interval] = {};
      if (typeof futuresKlineQueue[symbol] === 'undefined')
        futuresKlineQueue[symbol] = {};
      if (typeof futuresKlineQueue[symbol][interval] === 'undefined')
        futuresKlineQueue[symbol][interval] = [];
    ...
    let handleFuturesKlineStream = kline => {
      if (!kline)
        return console.error(`handleFuturesKlineStream: kline error`, kline);
      let symbol = kline.s,
        interval = kline.k.i;
      if (!futuresMeta[symbol] || !futuresMeta[symbol][interval].timestamp) {
        if (
          typeof futuresKlineQueue[symbol][interval] !== 'undefined' &&
          kline !== null
        ) {
    ...
    let getFuturesKlineSnapshot = async (symbol, limit = 500) => {
      ...
      if (options.verbose)
        console.info('getFuturesKlineSnapshot', symbol, limit, data);
      futuresKlineData(symbol, interval, data);
      //options.log('/futures klines at ' + futuresMeta[symbol][interval].timestamp);
      if (typeof futuresKlineQueue[symbol][interval] !== 'undefined') {
        for (let kline of futuresKlineQueue[symbol][interval])
          futuresKlineHandler(
            symbol,
            kline,
            futuresMeta[symbol][interval].timestamp
          );
  ...
  depthCache: function depthCacheFunction(symbols, callback, limit = 500) {
    ...
    let symbolDepthInit = symbol => {
      if (typeof depthCacheContext[symbol] === 'undefined')
        depthCacheContext[symbol] = {};
      let context = depthCacheContext[symbol];
  ...
  chart: function chart(symbols, interval, callback, limit = 500) {
    ...
    let handleKlineStreamData = kline => {
      ...
      if (!info[symbol] || !info[symbol][interval].timestamp) {
        console.warn(`${symbol} no info`, info[symbol], info[symbol][interval]);
        if (kline !== null) {
          //typeof klineQueue[symbol] !== 'undefined' && typeof klineQueue[symbol][interval] !== 'undefined' &&
          klineQueue[symbol][interval].push(kline);
        }
      } else {
        if (options.verbose) options.log('spot @klines at ' + kline.k.t);
    ...
    let getSymbolKlineSnapshot = (symbol, limit = 500) => {
      publicRequest(base + 'v3/klines', { symbol, interval, limit }, function(
        data
      ) {
        klineData(symbol, interval, data);
        //options.log('/klines at ' + info[symbol][interval].timestamp);
  ...
  bookTickers: function bookTickerStream(
    ...
    let reconnect = () => {
      if (options.reconnect) bookTickerStream(symbol, callback);
    };
    const endpoint = symbol
      ? `${symbol.toLowerCase()}@bookTicker`
      : '!bookTicker';
